...
| RFC-6002 | Doğrulama Kapılarının Kapsamı: `ratified` ≠ `canon` | **draft v0.2.0** (2026-07-27 — v0.1.0 iki bağımsız incelemeden de `wounded` aldı [`SKR-047` scientific + `ARCH-0001` architectural] ve **ikisi de aynı** bloke edici bulguya vardı: temel taşı `maturity-model.md:34` bayattı, RFC-6001 onu aşmıştı. v0.2.0 §2'yi **iki-yollu canon** modeline göre yeniden yazdı [kurucu yol = ratifikasyon, ampirik yol = M5; `ENS-0000:107-115` + `RFC-6001:341-345`]. "İki bağımsız türetme" iddiası **geri çekildi** [ortak kaynak + ardışıklık]. R2 "farklı aktör"den **ayrı edim**e çevrildi [CEO-\*/STYLE-SIGNOFF deseni, Faz-3'te 3/3]. §4'e tek-aktör kaçağını kapatan iki koruma cümlesi. **Kabul sırası tersine: RFC-6003 → RFC-6002 → RFC-6004** — 6002 tek başına kabul edilirse G4-boyut uyumu hiçbir yeni doğrulama yapılmadan %0→~%40 sıçrar, yani sistem gevşer. Yeni skeptic + farklı-boyut turu bekliyor.) |
...
| RFC-6004 | Madde V'e Güvenlik Niteliği: `Fail-safe` + `Mediated` | **draft** (v0.1.0, 2026-07-27 — **anayasa değişikliği**, Madde XV. Madde V'in nitelik listesinde güvenlik YOK; `ADR-0001` oradan "güvenli" diye tırnaklı alıntı yapıyordu (SCAN-01). **`Secure` bilerek REDDEDİLDİ** — Herley 2016 PNAS 113(23):6415-6420 (doğrulandı): güvenlik gereklilik-iddiaları yanlışlanamaz; `Secure` Madde V'in tek yanlışlanamaz üyesi olurdu ve Madde X ile çarpışırdı. Yerine iki DAR, yanlışlanabilir nitelik: `Fail-safe` (Saltzer&Schroeder fail-safe defaults; ISO/IEC 25010:2023 Safety alt-karakteristiği, doğrulandı) ve `Mediated` (complete mediation — DEFECT-REGISTER'ın 8 kalıbından 4'ü bunun türevi). Çatışma kuralı yeniden yazılıyor: asıl açık `Explainable`ın **muhatabının** tanımsız olması. **RFC-6003'e bağımlı** — G4'ün boyut şartını uygulayacak kadro atanmadan tam yoldan geçemez. skeptic + mimari + CEO + style zinciri bekliyor.) |
| RFC-6005 | Onay Makamı: eksik değil, **adsız** | **draft v0.1.0** (2026-07-27 — `RFC-6002` §3.1'in devrettiği aktör sorusunu karşılar. **Bootstrap paradoksu:** RFC kabul edilmek için onay makamı ister, makam RFC ile atanır. **Kırılışı:** makam yaratılmıyor, **var olan pratik tanınıyor** — onay edimi ENS'te dört kez yapıldı ve çalıştı (CEO-0001/0002/0003 + STYLE-SIGNOFF-RFC-6001). Ölçüm: Faz-3'te onaylayanı kayıtlı yapıt **3/3**, Faz 0-2'de **0/9**. **Çekirdek karar:** G3'ün istediği ayrım kişiler arası değil **edimler arası** (`ARCH-0001` bulgusu) — A1 ayrı kayıt, A2 ayrı context, A3 iki yönlü iz, **A4 onaylayan o yapıtın hiçbir doğrulama turunu yazmamış**. İki onay ekseni: `ens-ceo` (hiza) + `ens-style-guardian` (şema). Geriye dönük uygulanmaz — toplu onay onayın anlamını boşaltır. **fc-1 DOĞRULANMADI:** CEO-\* kayıtlarının context bağımsızlığı sınanmadı; ilk turun ilk işi bu.) |
...
| ADR-0003 | Kernel Invariant Hardening (K-3/K-4/K-5/K-6) | **draft v0.5.0** (2026-07-27 — dört sürüm, iki gerçek boyut (`SKR-049`+`SKR-050` scientific · `ENG-0001`+`ENG-0002` engineering). **BÖLÜNDÜ**: K-1→ADR-0004, K-2→ADR-0005. Kalan dört karar sonlanma ölçütünü sağladı: *iki ardışık turda yeni bloke edici bulgu yok*. Kapanma iddiası **22 kusur** (DP3+DP4+DP6+DP7). K-3 koşullu — analyzer'ın üç sessiz-başarısızlık yüzeyi Faz 0 işi. D-6 düzeltildi: `CS8618` `default(T)`'yi kapatmıyor, `Nullable` kategorisi gerekiyor; OQ1 yeniden açık.) |
| ADR-0004 | Güven Sınırı **Process**tir: in-process yetki korunamaz | **draft v0.1.0** (2026-07-27 — ADR-0003'ün K-1'i + DP5 devredildi; **kapanış belgesi, iterasyon değil**. `Unsafe.As` mührü kırıyor: reflection YOK, `AllowUnsafeBlocks=false`, registry'nin **kendi nesnesi yerinde** değişiyor, `ReferenceEquals` geçiyor çünkü aynı nesne. Oturum sahibi bağımsız doğruladı. Sonuç: güven sınırı çağrı grafiği değil **process**. K-1'in hedefi küçülür — mühür *kazayla/gündelik* taklidi durdurur, **kararlı saldırganı durdurmaz ve durduğunu iddia etmez**. `RFC-6004`'ün `Mediated` niteliğine talep açıldı. skeptic + mühendislik turu bekliyor.) |
| ADR-0005 | Kanonik Kimlik: harf katlama, Unicode, homoglyph | **draft v0.1.0** (2026-07-27 — ADR-0003'ün K-2'si devredildi; **iki turda da kırıldı**. v0.1.0 `ToUpperInvariant` Türkçe'de yanlış (ölçüldü); v0.3.0 reddetme `G4`'ü kapatmıyor + IDNA2003→2008 emsali aleyhte (ekosistem taşıyamadı, UTS #46 mapping'i geri getirdi). Ders: **tek profil yetmiyor** — eşleştirme katlama ister, Türkçe katlamama ister. Aday: **PRECIS iki-profil** (RFC 8265). `W2c` **kapsama geri alındı** — mixed-script kısıtı `UnicodeRanges` ile kurulabilir, D-3 bir kaçıştı. Türkçe katlama **tanımlı** (`CaseFolding.txt` `T` statüsü), BCL API'si yok.) |
